...
export default function NewServicePage() {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.message || "Gagal menambah layanan.");
      }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Tambah Layanan</h1>
          <p className="text-slate-500 font-medium text-sm">Definisikan layanan laundry baru dan harganya.</p>
        </div>
      </div>
      ...
      <Card className="border-none shadow-[0_2px_10px_rgba(0,0,0,0.02)] rounded-[32px] bg-white overflow-hidden">
        <CardHeader className="p-8 border-b border-slate-50 bg-slate-50/30">
          <div className="flex items-center gap-4">
            ...
            <div>
              <CardTitle className="text-lg font-bold text-slate-800">Detail Layanan</CardTitle>
              <CardDescription className="font-medium text-slate-400">Pastikan harga sudah sesuai dengan standar operasional.</CardDescription>
            </div>
          </div>
        </CardHeader>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Contoh: Cuci Kiloan Reguler" className="h-12 rounded-xl border-slate-100 bg-slate-50 focus-visible:ring-1 focus-visible:ring-[#0ddff2] font-medium px-4" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Contoh: Proses 2-3 hari kerja" className="h-12 rounded-xl border-slate-100 bg-slate-50 focus-visible:ring-1 focus-visible:ring-[#0ddff2] font-medium px-4" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
